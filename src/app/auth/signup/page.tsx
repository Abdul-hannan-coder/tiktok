...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to connect page after successful signup
            ...
            <Card className="bg-[#1A103D]/30 backdrop-blur-sm border-0 shadow-2xl shadow-[#6C63FF]/50 ring-0">
              ...
              <CardContent className="p-4">
                <form onSubmit={handleSubmit} className="space-y-6">
                  {/* Error Alert */}
                  {state.error && (
                    <div className="bg-[#FF2E97]/10 border border-[#FF2E97]/50 rounded-lg p-3 flex items-start space-x-3">
                      <AlertCircle className="h-5 w-5 text-[#FF2E97] flex-shrink-0 mt-0.5" />
                      <div className="flex-1">
                        <p className="text-[#FF2E97] text-sm font-medium">Signup Failed</p>
                        <p className="text-[#C5C5D2] text-sm mt-1">{state.error.message}</p>
                      </div>
                    </div>
                  )}
                  ...
                  <Button
                    type="submit"
                    disabled={!formData.agreeToTerms || formData.password !== formData.confirmPassword || state.status === 'loading'}
                  ...
                  >
                    {state.status === 'loading' ? 'Creating Account...' : 'Start Free Trial'}
                  </Button>
